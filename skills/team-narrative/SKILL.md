...
sub-agents spawned as a Codex subagent. This orchestrator does not write files
directly. Each sub-agent writes only within its owned paths (`rules/subagent-
collaboration.md` § 3) and returns the paths written; list every path with its
revert command (`git checkout -- <path>`) in the final report.
...
If the pipeline stops on K1/K2 (e.g., a lore contradiction that reverses an Accepted canon entry, or a prerequisite no skill in the repo produces):
