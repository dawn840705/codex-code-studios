...
입력이 파일이면 결과를 파일에 반영하고 보고한다. 묻지 않는다. 사용자 글이므로
되돌리기 경로가 있는지로 쓰는 자리를 정한다:

- 입력 파일이 **git 추적 중이고 깨끗하면**(`git status --porcelain -- <경로>` 출력 없음)
  제자리에 덮어쓴다. diff 요약과 `git checkout -- <경로>`를 보고한다.
- **추적되지 않거나 수정 중이면** 옆에 `<이름>.humanized.<확장자>`로 쓰고 경로를 보고한다.
  원본은 손대지 않는다.
- 입력이 붙여넣은 텍스트면 응답으로만 보여준다.
- `_workspace/` 아래 중간 산출물(`01_input_with_metrics.txt` 등)은 작업 파일이다.
