...
**Preserve byte-exact raw evidence.** When original bytes are required (including CSV or text with mixed line endings), record the external source hash and exclude the exact paths from Git text normalization before import, using `binary` or `-text` attributes; `eol=lf` is not byte preservation. Attribute changes alone or an ordinary `git add` may leave previously normalized index bytes in place. Restore the verified original bytes, re-ingest only those paths with `git add --renormalize -- <paths>`, and check the effective attributes and index content. Export the staged files to a fresh directory with `git checkout-index` and compare their actual hashes with the independent originals before committing. Matching working-tree hashes or an empty diff alone do not prove receiving-checkout fidelity. Keep raw evidence unchanged; store normalized working copies separately and repair history with a new scoped commit rather than rewriting it.
