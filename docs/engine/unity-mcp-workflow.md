...
### 3.10 열린 씬의 외부 변경과 지연된 Reload 대화 상자

- 엔진이 저장한 scene/prefab/animation YAML을 공백·개행 검사 때문에 다시 쓰지 않는다. 엔진 직렬화의 공백은 해당 생성 파일의 검사 범위에서 다루고 전역 검사나 자동 새로고침을 끄지 않는다.
- 열린 scene 파일을 외부 패치·복사·Git 통합으로 변경해야 하면 dirty 상태를 확인하고 소유 작업을 저장한 뒤 대상 scene을 닫는다. 외부 변경→재임포트→명시적 열기를 순차 수행한다. 사용자 미저장 상태는 보존한다.
- 같은 바이트의 재복사·형식 정리를 피한다. 외부 변경 직후 `isDirty=false`여도 다음 focus/refresh에서 Reload 모달이 지연 표시될 수 있으므로 그 값을 디스크 동기화의 증거로 쓰지 않는다.
- 도구 응답이 멈추면 현재 작업 ID와 Editor 상태·대화 상자를 확인한다. 타임아웃만으로 동일 변경을 재요청하거나 사용자 씬을 자동으로 덮어쓰지 않는다.
