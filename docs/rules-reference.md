...
| Rule File | Enforces |
| ---- | ---- |
| `autonomy-contract.md` | **Default is proceed.** The only stops are R4 / paid calls, an unresolved track (ask once, write `production/track.txt`), overturning a pinned decision, files outside write ownership, facts only a person can observe (batched into the exit report), and items the user explicitly asked to review. Proceeding carries duties: record route and R grade, leave an undo path, prove with exit codes, no rewrite without a defect ticket. Subagents return decision items, not questions; a skill is an artifact contract, not a procedure script. Canonical copy; `docs/AGENTS-template.md` and `CLAUDE.md` carry it verbatim |
...
| `subagent-collaboration.md` | Multi-subagent parallel collaboration pattern for broad/deep design decisions — when to fan out, prompt requirements, result integration, meeting minutes. § 2.1: **write ownership is partitioned before spawning** — two agents never get the same file, because a lost Edit is the one failure mode that reports success. § 2.2–2.3: consultation order for dependent work and the parallel protocol (spawn all, surface BLOCKED at once, partial report). § 3.2/§ 4: subagents have no user channel — they return **decision items** (problem / recommendation / alternatives / evidence / undo cost), the orchestrator decides inside `autonomy-contract.md`, and only K1 items go to the user, batched |
